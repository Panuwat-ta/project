...
def process_and_save(file_list, img_out_dir, ann_out_dir):
    ...
    for i, (img_path, label) in iterator:
        ...
        if label == 1:  # Tampered
            # หา folder i จาก img_path
            # img_path = .../splicing_{i}_img/img/...
            img_dir = os.path.dirname(os.path.dirname(img_path)) # .../splicing_{i}_img
            splicing_i_img = os.path.basename(img_dir)
            splicing_i_ann = splicing_i_img.replace('_img', '_annotations')
            base_dir = os.path.dirname(img_dir) # .../defacto-splicing
            
            donor_path = os.path.join(base_dir, splicing_i_ann, 'donor_mask', raw_name + '.tif')
            probe_path = os.path.join(base_dir, splicing_i_ann, 'probe_mask', raw_name + '.tif')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Prepare splicing dataset for SegFormer')
    parser.add_argument('--splicing-dir', required=True, help='Base directory for defacto-splicing')
    ...
    if not os.path.isdir(args.splicing_dir):
        raise FileNotFoundError(f"Splicing directory not found: {args.splicing_dir}")
    ...
    tp_files = []
    for i in range(1, 8):
        tp_dir = os.path.join(args.splicing_dir, f'splicing_{i}_img', 'img')
        if os.path.isdir(tp_dir):
            tp_files.extend(sorted(glob.glob(os.path.join(tp_dir, '*.*'))))

    if len(tp_files) == 0:
        raise ValueError(f"No tampered images found in: {args.splicing_dir}")
