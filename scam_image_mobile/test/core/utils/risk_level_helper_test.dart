...
void main() {
  group('RiskLevelHelper.factorLevelForScore', () {
    // -- Low boundary (0-39) -------------------------------------------

    test('score 0 -> RiskLevel.low', () {
      expect(RiskLevelHelper.factorLevelForScore(0), RiskLevel.low);
    });

    test('score 1 -> RiskLevel.low', () {
      expect(RiskLevelHelper.factorLevelForScore(1), RiskLevel.low);
    });

    test('score 19 -> RiskLevel.low (upper boundary of low low-range)', () {
      expect(RiskLevelHelper.factorLevelForScore(19), RiskLevel.low);
    });
    // -- Low boundary (20-39) -----------------------------------------

    test('score 20 -> RiskLevel.low (lower boundary of low)', () {
      expect(RiskLevelHelper.factorLevelForScore(20), RiskLevel.low);
    });

    test('score 30 -> RiskLevel.low (mid-range)', () {
      expect(RiskLevelHelper.factorLevelForScore(30), RiskLevel.low);
    });

    test('score 39 -> RiskLevel.low (upper boundary of low)', () {
      expect(RiskLevelHelper.factorLevelForScore(39), RiskLevel.low);
    });

    // -- Medium boundary (40-69) ----------------------------------------

    test('score 40 -> RiskLevel.medium (lower boundary of medium)', () {
      expect(RiskLevelHelper.factorLevelForScore(40), RiskLevel.medium);
    });

    test('score 55 -> RiskLevel.medium (mid-range)', () {
      expect(RiskLevelHelper.factorLevelForScore(55), RiskLevel.medium);
    });

    test('score 69 -> RiskLevel.medium (upper boundary of medium)', () {
      expect(RiskLevelHelper.factorLevelForScore(69), RiskLevel.medium);
    });

    // -- High boundary (70-100) ----------------------------------------

    test('score 70 -> RiskLevel.high (lower boundary of high)', () {
      expect(RiskLevelHelper.factorLevelForScore(70), RiskLevel.high);
    });

    test('score 90 -> RiskLevel.high (mid-range)', () {
      expect(RiskLevelHelper.factorLevelForScore(90), RiskLevel.high);
    });

    test('score 100 -> RiskLevel.high (max)', () {
      expect(RiskLevelHelper.factorLevelForScore(100), RiskLevel.high);
    });
  });
  ...
  group('RiskLevelHelper.toLabelKey', () {
    ...
    test('medium returns result_medium_risk', () {
      expect(
        RiskLevelHelper.toLabelKey(RiskLevel.medium),
        'result_medium_risk',
      );
    });
  ...
  group('RiskLevelHelper.toIcon', () {
    ...
    test('high returns warning_amber_rounded', () {
      expect(
        RiskLevelHelper.toIcon(RiskLevel.high),
        Icons.warning_amber_rounded,
      );
    });

    test('unknown returns help icon, never a risk color', () {
      expect(
        RiskLevelHelper.toIcon(RiskLevel.unknown),
        Icons.help_outline_rounded,
      );
      expect(RiskLevelHelper.toThaiLabel(RiskLevel.unknown), 'Unknown');
      expect(
