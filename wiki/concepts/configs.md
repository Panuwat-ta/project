...
* **$S_{source}$** คือ คะแนนความเสี่ยงจากแหล่งที่มาของภาพ (Source Reliability Score 0–100%) จาก Reverse Image Search (ที่มา: นโยบาย heuristic v1 — พบ ≥3 แหล่ง = สูง, =2 = ปานกลาง/ไม่แน่ชัด, ≤1 = ต่ำ; ทบทวนเกณฑ์เมื่อมีข้อมูลภาคสนาม)
...
* **ช่วงอันตราย (70 -- 100):** ผลวิเคราะห์ส่วนใหญ่ชี้ไปในทิศทางเดียวกันว่าภาพถูกปรับแต่งหรือมีข้อความหลอกลวงที่ชัดเจน ภาพนี้มีความเสี่ยงสูงที่จะเป็นสแกม (กรณี $S_{visual} \ge 80$ ระบบจะขึ้นเป็น High ทันที)
...
* **Normalize:** นิยามตายตัว $\text{Normalize}(y) = \min(100, \text{round}(y \times 100))$ — แปลงผลคูณ (ช่วง 0–1) เป็นคะแนน 0–100 (ฉบับ canonical ดู `Document/model/configs.md` §3)
...
* **ตัวอย่างคำนวณ:** Confidence เฉลี่ย 0.9, Coverage 0.2 → $0.9 \times 0.2 = 0.18$ → $\text{Normalize}(0.18) = 18$ → $S_{visual} = 18$ (Low)
