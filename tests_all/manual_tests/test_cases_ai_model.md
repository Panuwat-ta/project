...
- **Expected Results**:
  1. แผนที่ความร้อนความละเอียดเต็ม (Full-Res Heatmap) ต่อเนื่องไร้รอยต่อ: ค่าเฉลี่ยบริเวณทับซ้อนต่างจากค่าพิกเซลข้างเคียงไม่เกิน ±0.05
  2. ไม่ปรากฏรอยตะเข็บรูปสี่เหลี่ยมตามขอบของ Patch (Seamless Patch Merging)
  3. บริเวณที่มีการตัดต่อตรงรอยต่อพิกเซลมีค่าเฉลี่ยความน่าจะเป็น (mean prob) ≥0.7 และสูงกว่าพื้นหลังข้างเคียง ≥0.3
- **Automation Mapping**: Manual Verification
...
- **Expected Results**:
  1. สร้างข้อความสรุปภาษาไทย 1-2 ประโยค เข้าใจง่าย (rubric ข้อ 1: ความยาวและภาษา)
  2. ชี้แจงเหตุผลชัดเจน เช่น "ตรวจพบความผิดปกติของพิกเซลบริเวณยอดเงิน และมีลักษณะการวางทับข้อความเดิม" โดย region/score/keywords ต้องตรงกับอินพุต SegFormer+OCR จริง (rubric ข้อ 2: ความสอดคล้องกับอินพุต)
  3. ไม่มีการเพ้อเจ้อ (Hallucination) ข้อมูลที่อยู่นอกเหนือจากผลการตรวจจับจริง — ทุกประโยคต้องอ้างอิงได้จากผลตรวจจับ (rubric ข้อ 3: ตรวจสอบย้อนกลับได้); เกณฑ์ผ่านคือ rubric ครบ 3 ข้อโดยผู้ตรวจอิสระ 2 คนเห็นตรงกัน
...
- **Expected Results**:
  1. Main FastAPI Process ไม่แครชตาม และ `GET /health` ตอบ `200 OK` ภายใน 2 วินาทีหลัง Subprocess ล้มเหลว
  2. คำขอถัดไปสามารถสร้าง Subprocess ใหม่เพื่อประมวลผลได้ (ตรวจด้วย `GET /health` ตอบ 200 ภายใน 2s เช่นกัน)
  3. Client ได้รับข้อความแจ้งเตือนข้อผิดพลาดที่เหมาะสม (JSON `{"detail": ...}` ไม่ใช่ HTML/timeout) ไม่เกิด Connection Hang (TCP ไม่ค้างเกิน 10s)
- **Automation Mapping**: `tests_all/automate_tests/tests/api/test_scan_workflow.py`
...
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  ...
  2. ไม่พบรอยตะเข็บตามขอบ Patch (ค่าบริเวณทับซ้อนต่างจากข้างเคียงไม่เกิน ±0.05)
  3. บริเวณตัดต่อมีค่าเฉลี่ยความน่าจะเป็น (mean prob) ≥0.7 และสูงกว่าพื้นหลังข้างเคียง ≥0.3
...
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  ...
  2. ภาพเอียงให้ Character Accuracy ไม่ต่ำกว่า 70% ของภาพอ้างอิง ภาพเบลอไม่ต่ำกว่า 50% ของภาพอ้างอิง (ต่ำกว่านี้ถือว่า FAIL — เกณฑ์ตก)
...
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  ...
  2. บริเวณกรอบเฉลยมีค่าเฉลี่ยความน่าจะเป็น (mean prob) ≥0.7 และสูงกว่าบริเวณปกติ ≥0.3
...
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  ...
  2. คำอธิบายเป็นภาษาไทย 1-2 ประโยค ระบุว่าไม่พบข้อความน่าสงสัย (rubric เดียวกับ TC-AI-XAI-01: ครบ 3 ข้อโดยผู้ตรวจ 2 คน)
...
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  ...
  2. คะแนนภาพอ้างอิงเบี่ยงเบนไม่เกิน |Δ| ≤5 คะแนน และระดับ low/medium/high ไม่ข้ามเกรด (ข้ามเกรดถือว่า FAIL)
...
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  ...
  2. คำอธิบายยังเป็นภาษาไทย 1-2 ประโยคตามผลคะแนนจริง (rubric เดียวกับ TC-AI-XAI-01: region/score/keywords ตรงอินพุต ผู้ตรวจ 2 คน)
