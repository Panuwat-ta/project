...
- **Expected Results**:
1. สถานะนี้เป็น GAP บางส่วนฝั่งหน้าจอ: ไม่พบหน้าจอคำเตือน 10MB ในโค้ดปัจจุบัน (`ScanRepositoryImpl` มีเพียงค่าคงที่ `_maxFileSizeBytes` 10MB กับ `assert(print)` เตือนเฉพาะโหมดดีบัก ไม่มีแถบแจ้งเตือนฝั่งหน้าจอให้ผู้ใช้เห็น)
2. การบีบอัดใช้ `imageQuality: 85` ของ `ImagePicker` กับ `compressQuality: 85` ของ `ImageCropper` แบบคงที่ทุกไฟล์ ไม่ได้บีบอัดเฉพาะไฟล์เกิน 10MB และแอปไม่ปฏิเสธไฟล์ทันที
...
- **Expected Results**:
  ...
  2. สถานะนี้เป็น GAP ส่วนย่อยฝั่งหน้าจอ: ไม่พบแถบแจ้งเตือนสีส้มหรือข้อความโหมดออฟไลน์ในโค้ดปัจจุบัน (`history_repository_impl` มีแคชและโหลดแบบออฟไลน์เฟิร์ส แต่ไม่มีวิดเจ็ตแบนเนอร์ออฟไลน์) การแสดงรายการจากแคชทำได้ แต่ไม่มีแถบแจ้งเตือนตามข้อความเดิม
...
- **Test Steps (ขั้นตอนการทดสอบ)**:
  1. ทดสอบทีละเคส: กรอกตามเคสแล้วกดปุ่มสมัครสมาชิก
  2. สังเกตข้อความกำกับใต้ช่องกรอกและพฤติกรรมของปุ่ม
  3. เคสอีเมลซ้ำ: กรอกข้อมูลถูกต้องทุกช่อง กดยอมรับเงื่อนไข แล้วกดสมัคร
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  1. ช่องที่ว่างแสดงข้อความแจ้งให้กรอก และไม่มีคำขอส่งออกไป
  2. รหัสผ่านสั้นกว่า 8 ตัวอักษรถูกปฏิเสธที่ฟอร์ม พร้อมข้อความแจ้งความยาวขั้นต่ำ
  3. ช่องยืนยันรหัสที่ไม่ตรงกันแสดงข้อความแจ้งว่ารหัสไม่ตรงกัน
  4. ไม่กดยอมรับเงื่อนไขแล้วกดสมัคร แอปแสดงแถบแจ้งเตือนสีแดงและไม่ส่งคำขอ
  5. อีเมลซ้ำถูกปฏิเสธจากฝั่งบริการ พร้อมข้อความว่าอีเมลนี้ถูกใช้งานแล้ว โดยผู้ใช้ยังอยู่ที่หน้าลงทะเบียน (หมายเหตุ: หน้าลงทะเบียนตรวจเฉพาะค่าว่าง ส่วนการตรวจรูปแบบอีเมลด้วยนิพจน์ปรกติมีที่หน้าลงชื่อเข้าใช้)
- **Automation Mapping**: Manual UI Test
...
- **Test Data (ข้อมูลที่ใช้ทดสอบ)**:
  - รายละเอียด `โกง` (สั้นกว่า 5 ตัวอักษร)
  - รายละเอียด 6 ตัวอักษร เช่น `โกงเงินครับ`
  - กรณีไม่เลือกหมวดหมู่
- **Test Steps (ขั้นตอนการทดสอบ)**:
  1. ไม่เลือกหมวดหมู่แล้วกดส่ง
  2. เลือกหมวดหมู่ กรอกรายละเอียดสั้นกว่า 5 ตัวอักษร แล้วกดส่ง
  3. กรอกรายละเอียด 6 ตัวอักษร แล้วกดส่ง
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  1. ไม่เลือกหมวดหมู่มีข้อความแจ้งให้เลือกหมวดหมู่ และไม่มีคำขอส่งออกไป
  2. รายละเอียดสั้นกว่า 5 ตัวอักษรถูกปฏิเสธที่ฟอร์ม พร้อมข้อความแจ้งให้กรอกเพิ่ม
  3. รายละเอียด 6 ตัวอักษรผ่านฟอร์มฝั่งแอป แต่ถูกปฏิเสธที่ฝั่งบริการ (เกณฑ์ขั้นต่ำ 10 ตัวอักษร) พร้อมแถบแจ้งข้อผิดพลาดสีแดง โดยแอปไม่แครช
- **Automation Mapping**: Manual UI Test
...
- **Priority**: P2 (Medium)
- **Pre-conditions (เงื่อนไขก่อนเริ่มทดสอบ)**:
  1. เตรียมอุปกรณ์ Android รุ่นเก่าหนึ่งเครื่องและรุ่นใหม่หนึ่งเครื่องตามรุ่นขั้นต่ำที่โปรเจกต์รองรับ
  2. ติดตั้งแอปรุ่นเดียวกันทั้งสองเครื่อง
- **Test Data (ข้อมูลที่ใช้ทดสอบ)**:
...
- **Test Steps (ขั้นตอนการทดสอบ)**:
  ...
  3. ลองเปิดลิงก์ผลสแกนจากภายนอกแอป (เช่น แตะลิงก์จากแอปแชท)
- **Expected Results (ผลลัพธ์ที่คาดหวัง)**:
  1. ประวัติเดิมยังแสดงครบ แตะแล้วเปิดผลเดิมได้ข้อมูลตรง
  2. การนำทางภายในแอปไปหน้ารายงานผลและรายละเอียดยังทำงานปกติ
  3. การเปิดลิงก์จากภายนอกแอปเป็น GAP เนื่องจากไม่พบการรองรับลิงก์ระดับระบบปฏิบัติการในรุ่นปัจจุบัน
- **Automation Mapping**: Manual Device Test
