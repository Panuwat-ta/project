...
- **test_qwen_xai_explanation_generation**:
  - พฤติกรรมที่ผ่าน: ส่ง context จำลอง (`region` = บริเวณข้อความยอดเงินและตราประทับ, `visual_score` = 85, `ai_gen_probability` = 0.90, `scam_keywords` = ยอดเงิน/โอนเงินสำเร็จ) เข้า `generate_xai_explanation()` แล้วได้ผลลัพธ์เป็นข้อความภาษาไทยที่มีความยาวมากกว่า 10 ตัวอักษร โดย oracle ตรวจว่า (1) เอาต์พุตอ้างถึง region อินพุต (ยอดเงิน/ตราประทับ), (2) อ้างถึงคะแนนช่วง High (visual 85 → เกรด high), (3) มี keywords ตรงอินพุต (ยอดเงิน/โอนเงิน) อย่างน้อย 1 คำ และ (4) ไม่ตกกลับไปใช้ข้อความ fallback กลาง (`ไม่พบร่องรอยการตัดต่อ...`) แสดงว่าโมเดลสร้างคำอธิบายเชิงเหตุผลได้จริง
