...
const bootstrap=`(()=>{
  ...
  const userDetail={...userSummary,total_reports:1,recent_scans:[{id:'scan-1',total_risk_score:42,created_at:'2026-09-20T10:00:00Z'}]};
  ...
  for(const theme of themes){
    for(const [route,expectedHeading] of routes){
      ...
      const info=await evaluate(`(async()=>{
        ...
        return {path:location.pathname,title:document.title,h1:document.querySelector('h1')?.textContent?.trim()||null,h2:document.querySelector('h2')?.textContent?.trim()||null,theme:root.classList.contains('dark')?'dark':root.classList.contains('light')?'light':'unknown',overflow:root.scrollWidth>root.clientWidth+1,scrollWidth:root.scrollWidth,clientWidth:root.clientWidth,unexpected:text.includes('Unexpected Application Error'),loadError:/ไม่สามารถโหลด|เกิดข้อผิดพลาดในการโหลด/.test(text),alerts:visibleError,webmcp,bodySample:text.slice(0,160)};
      })()`);
      ...
      const pass=!info.exception && value.path===route && value.theme===theme && !value.overflow && !value.unexpected && !value.loadError && runtimeExceptions.length===0 && consoleErrors.length===0 && value.webmcp?.available && value.webmcp?.toolFound && web?.path===route && web?.theme===theme && web?.heading===expectedHeading;
