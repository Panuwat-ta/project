...
* **$S_{source}$:** คะแนนความเสี่ยงจากแหล่งที่มาของภาพ (Source Reliability Score) จาก Reverse Image Search (ที่มา: นโยบาย heuristic v1 — พบ ≥3 แหล่ง = สูง, =2 = ปานกลาง/ไม่แน่ชัด, ≤1 = ต่ำ; ที่มาของเลข 3/2/1: เกณฑ์แยกกรณีเบื้องต้นแบบ conservative — 1 แหล่งอาจเป็นต้นฉบับเอง, 2 แหล่งไม่พอสรุป, ≥3 แหล่งบ่งชี้การแพร่กระจาย; trigger ทบทวนตัวเลข: เมื่อมีข้อมูลภาคสนาม n≥100 เคส หรืออัตราตัดสินผิดทิศทาง >10% ให้ปรับ threshold ใหม่พร้อมบันทึกมติ)
...
* **Normalize:** นิยามตายตัว $\text{Normalize}(y) = \min(100, \text{round}(y \times 100))$ — แปลงผลคูณ (ช่วง 0–1) เป็นคะแนน 0–100
...
> สูตรนี้เป็นฉบับ canonical ของ $S_{visual}$ ทั้งระบบ (สูตร max-prob เดี่ยวที่เคยกล่าวไว้ใน `design/architecture.md` รุ่นก่อนถูกยกเลิกแล้ว)
