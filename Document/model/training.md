...
เอกสารฉบับนี้อธิบายรายละเอียดเกี่ยวกับกลยุทธ์การฝึกสอนโมเดล (Training Strategy) การอัปเดตน้ำหนักโมเดลอย่างต่อเนื่อง และการนำโมเดลไปใช้งาน (Deployment) ซึ่งเป็นส่วนต่อขยายจากข้อมูลใน `Document/model/model.md`
...
* **ความแม่นยำรวมของ AI (Accuracy และ mDice):** เป้าหมายอยู่ที่ความแม่นยำ >= 85% สำหรับการตรวจจับภาพตัดต่อและการแยกแยะจุดเสี่ยง — วัดบน held-out test set ที่ไม่ซ้ำ train/val (แบ่งแบบ stratified 80:10:10 คงสัดส่วน class แท้/ปลอมและแหล่งข้อมูล, seed คงที่, test set ถูก freeze ห้าม tune)
* **รอบทบทวน retrain:** ทุก 30 วัน หรือเมื่อมีภาพใหม่ที่ approved + research-consented สะสม ≥ 500 ภาพ (แล้วแต่เงื่อนไขใดถึงก่อน)
