...
flowchart TD
    ...
    subgraph Visual["Layer 2: Visual Anomaly Detection (วิเคราะห์ร่องรอยตัดต่อ)"]
        SegFormer["SegFormer ONNX<br>(Transformer Segmentation, subprocess)"]
        HeatmapGen["Post-Processing<br>(Mask-to-Heatmap Overlay)"]
    end
    ...
    NLP -->|"คะแนนความเสี่ยงด้านข้อความ"| Final["ประเมินระดับความเสี่ยงรวม<br>(Hybrid max+bonus Risk Score)"]
...
### Surya-OCR (Native PyTorch v0.5.0)
**Surya-OCR** เป็นโมเดลสำหรับอ่านตัวอักษรและวิเคราะห์โครงสร้างเอกสารสมัยใหม่ (Modern OCR) โดยในโปรเจกต์นี้เลือกใช้ในรูปแบบ **Native PyTorch (v0.5.0)** เพื่อให้มีความเสถียรสูงสุดและทำงานร่วมกับ Backend Python (FastAPI) ได้โดยตรง โดยไม่จำเป็นต้องพึ่งพาเซิร์ฟเวอร์แยก

> หมายเหตุ: โมเดลภาษาขนาดเล็กสำหรับ XAI reasoning จะแยกจากโมดูล OCR
...
คะแนนความเสี่ยงโดยรวม (Total Risk Score) คำนวณตามแนวทาง Hybrid Worst-Case Trigger ร่วมกับ Multi-Factor Compounding:
...
เกณฑ์การตัดสินระดับความเสี่ยง (Risk Grade — 3 ระดับ: Low 0-39 / Medium 40-69 / High 70-100):
...
* **ช่วงอันตราย (70 -- 100):** ผลวิเคราะห์ส่วนใหญ่ชี้ไปในทิศทางเดียวกันว่าภาพถูกปรับแต่งหรือมีข้อความหลอกลวงที่ชัดเจน ให้ถือว่าภาพนี้มีความเสี่ยงสูงที่จะเป็นสแกม (กรณี $S_{visual} \ge 80$ ระบบจะขึ้นเป็น High ทันที แม้ $S_{total} < 70$ โดยบังคับคะแนนขั้นต่ำ 70)
